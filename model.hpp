...
public:
	...
	vector <float> my_strok_f(char* str, char* delimeter) {
		vector <float> v;
		char* context;
	...
	vector <string> my_strok_s(char* str, char* delimeter) {
		vector <string> v;
		char* context;
		char* tok = strtok_s(str, delimeter, &context);
	...
	vector <int> my_strok_i(char* str, char* delimeter) {
		vector <int> v;
		char* context;
		char* tok = strtok_s(str, delimeter, &context);
	...
	void loadObj(ifstream& fin) {
		...
		while (getline(fin, line)) {
			...
			if (line[0] == 'v' && line[1] == ' ') {
				vf = my_strok_f((char*)line.substr(2, len - 2).c_str(), (char*)" ");
				p3.d = { vf[0],vf[1],vf[2] };
				...
				vf = my_strok_f((char*)line.substr(3, len - 3).c_str(), (char*)" ");
				p2.d = { vf[0],vf[1] };
				...
				s = my_strok_s((char*)line.substr(2, len - 2).c_str(), (char*)" ");
				int nVertexes = s.size();
				CFare face_tmp;
				for (int i = 0; i < nVertexes; i++) {
					vi = my_strok_i((char*)s[i].c_str(), (char*)"/");
